		...
		struct dmatest_thread_result *tr)
{
	...
	snprintf(buf, sizeof(buf) - 1, "%s: #%u: %s with src_off=0x%x "
		"dst_off=0x%x len=0x%x (%lu)", name, tr->n, messages[tr->type],
		tr->src_off, tr->dst_off, tr->len, tr->data);
		...
		bool is_srcbuf)
{
	...
	if (!vr) {
		pr_warning("dmatest: No memory to store verify result\n");
		...
		size_t count, loff_t *ppos)
{
	...
	if (strtobool(buf, &bv) == 0) {
		...
		/* Run test with new parameters if asked for */
		if (bv) {
