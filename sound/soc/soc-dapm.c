...
int snd_soc_dapm_connect_dai_link_widgets(struct snd_soc_card *card)
{
	...
	/* for each RTD DAI link... */
	for (i = 0; i < card->num_rtd; i++) {
		rtd = &card->rtd[i];
		cpu_dai = rtd->cpu_dai;
		codec_dai = rtd->codec_dai;

		/* dynamic links have no fixed DAI mapping */
		if (rtd->dai_link->dynamic)
			continue;

		/* there is no point in connecting DAI links with dummies */
		if (snd_soc_dai_is_dummy(codec_dai) ||
			snd_soc_dai_is_dummy(cpu_dai))
			continue;

		/* connect playback if widgets are valid */
