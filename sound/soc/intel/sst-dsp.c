...
void sst_dsp_write(struct sst_dsp *sst, void *src, u32 dest_offset,
	size_t bytes)
{
	sst->ops->ram_write(sst, sst->addr.ram + dest_offset, src, bytes);
}
...
void sst_dsp_read(struct sst_dsp *sst, void *dest, u32 src_offset,
	size_t bytes)
{
	sst->ops->ram_read(sst, dest, sst->addr.ram + src_offset, bytes);
}
...
{
	sst->mailbox.in_base = sst->addr.ram + inbox_offset;
	sst->mailbox.out_base = sst->addr.ram + outbox_offset;
