			      ...
			      struct snd_pcm_hw_params *params)
{
	...
	module_data = sst_module_get_config(dsp, module_id);
	if (module_data == NULL) {
		dev_err(rtd->dev, "PCM: failed to get module config\n");
		return -EINVAL;
	}
	sst_hsw_stream_set_module_info(hsw, pcm_data->stream,
		module_id, module_data->entry - 4);

	sst_hsw_stream_set_pmemory_info(hsw, pcm_data->stream,
		module_data->p.offset + 0x400000, module_data->p.size);

	sst_hsw_stream_set_smemory_info(hsw, pcm_data->stream,
		module_data->s.offset + 0x400000, module_data->s.size);
