...
static int hsw_acpi_resource_map(struct sst_dsp *sst, struct sst_pdata *pdata)
{
	...
	/* SST Shim */
	sst->addr.shim = sst->addr.dram + sst->addr.shim_offset;

	/* IRAM */
	sst->addr.iram_base = sst->addr.dram_base + sst->addr.iram_offset;
	sst->addr.iram = sst->addr.dram + sst->addr.iram_offset;
...
static int hsw_set_d0(struct sst_dsp *sst, struct acpi_device *acpi)
{
	int tries = 10, err, state;
	u32 reg;

	/* Set D0 state */
	acpi_device_get_power(acpi, &state);
	if (state != ACPI_STATE_D0) {

		err = acpi_device_set_power(acpi, ACPI_STATE_D0);
		if (err < 0)
			dev_err(sst->dev, "failed to set D0\n");
	}
...
static int hsw_init(struct sst_dsp *sst, struct sst_pdata *pdata)
{
	...
	dev = sst->dev;
	acpi = container_of(dev, struct acpi_device, dev);
