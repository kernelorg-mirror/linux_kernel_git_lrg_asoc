...
static int hsw_log_message(struct sst_hsw *hsw, u32 header)
{
	...
	if (down_interruptible(&stream->rw_mutex))
		return -ERESTARTSYS;
...
static ssize_t fw_log_copy_to_user(struct sst_hsw_log_stream *log_stream, char __user *user_buf,
					size_t count)
{
					...
					size_t count, loff_t *ppos)
{
	...
	do {
		if (down_interruptible(&log_stream->rw_mutex))
			return -ERESTARTSYS;
...
int sst_hsw_dbg_enable(struct sst_hsw *hsw, struct dentry *debugfs_card_root)
{
	...
	log_stream->size = 256 * PAGE_SIZE;
	log_stream->hsw = hsw;
	log_stream->dma_area = dma_alloc_coherent(hsw->dev, log_stream->size,
		&log_stream->dma_addr, GFP_KERNEL);
	dev_info(hsw->dev, "alloc dma buffer: area=%p, addr=%p, size=%d\n",
		(void *)log_stream->dma_area,
		(void *)log_stream->dma_addr,
		log_stream->size);

	if (!log_stream->dma_area) {
		dev_err(hsw->dev, "alloc dma buffer failed\n");
		return -EINVAL;
	}

	log_stream->ring_descr = kzalloc(PAGE_SIZE, GFP_DMA);
	if (!log_stream->ring_descr) {
		dev_err(hsw->dev, "alloc ring descriptor failed\n");
		return -EINVAL;
	}
	...
	dev_info(hsw->dev, "generating page table for %p size 0x%x pages %d\n",
	...
	sema_init(&log_stream->rw_mutex, 1);
	...
	if (!debugfs_create_u32("fw_log_size", 0644, debugfs_card_root,
				&log_stream->size))
		pr_warn("ASoC: Failed to create fw_log_size debugfs file\n");
