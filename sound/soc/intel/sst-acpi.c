...
#define SST_LPT_DSP_DMA_ADDR_MASK	0xFFF00000
#define SST_LPT_DSP_DMA_MAX_BURST	0x3
...
static int sst_acpi_probe(struct platform_device *pdev)
{
	...
	if (desc->resindex_dma_base >= 0) {
		...
		sst_pdata->dma_mask = desc->dma_mask;
		sst_pdata->dma_burst = desc->dma_burst;
	}
	...
	sst_pdata->hsw = sst_hsw_dsp_init(dev, sst_pdata);
	if (sst_pdata->hsw == NULL)
		return -ENODEV;

	/* register Haswell/Broadwell PCM and DAI driver */
	sst_acpi->pdev_pcm = platform_device_register_data(dev,
		"haswell-pcm-audio", -1,  sst_pdata, sizeof(*sst_pdata));
...
static int sst_acpi_remove(struct platform_device *pdev)
{
	...
	sst_hsw_dsp_free(sst_acpi->sst_pdata.hsw);
...
static struct sst_acpi_desc sst_acpi_haswell_desc = {
	...
	.dma_mask = SST_LPT_DSP_DMA_ADDR_MASK,
	.dma_burst = SST_LPT_DSP_DMA_MAX_BURST,
};
