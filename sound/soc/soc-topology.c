...
#define SOC_TPLG_PASS_COEFF		SOC_TPLG_PASS_MIXER
#define SOC_TPLG_PASS_WIDGET		3
#define SOC_TPLG_PASS_GRAPH		4
#define SOC_TPLG_PASS_PINS		5
#define SOC_TPLG_PASS_FE		6
#define SOC_TPLG_PASS_BE		7
#define SOC_TPLG_PASS_CC		8

#define SOC_TPLG_PASS_START	SOC_TPLG_PASS_MANIFEST
#define SOC_TPLG_PASS_END	SOC_TPLG_PASS_CC
...
struct soc_tplg {
	...
	u32 id;		/* current block ID */
	u32 req_id;	/* required ID, only loaded/free matching blocks */
	...
	/* dai operations */
	const struct snd_soc_tplg_dai_ops *dai_ops;
	int dai_ops_count;
...
static const struct snd_soc_tplg_kcontrol_ops io_ops[] = {
	...
	{SOC_CONTROL_IO_BYTES_EXT, NULL,
		NULL, snd_soc_bytes_info_ext},
	{SOC_CONTROL_IO_ENUM_VALUE, snd_soc_get_enum_double,
	...
	{SOC_DAPM_IO_ENUM_VIRT, snd_soc_dapm_get_enum_double,
		snd_soc_dapm_put_enum_double, NULL},
	{SOC_DAPM_IO_ENUM_VALUE, snd_soc_dapm_get_enum_double,
		snd_soc_dapm_put_enum_double, NULL},
...
static void soc_bind_err(struct soc_tplg *tplg, int idx, int count)
{
	dev_err(tplg->dev,
		"ASoC: invalid control type %d:%d:%d count %d\n",
		SOC_CONTROL_GET_ID_GET(idx), SOC_CONTROL_GET_ID_PUT(idx),
		SOC_CONTROL_GET_ID_INFO(idx), count);
}

static void soc_control_err(struct soc_tplg *tplg, const char *name, int idx)
{
	dev_err(tplg->dev,
		"ASoC: no complete mixer IO handler for %s type (g,p,i) %d:%d:%d\n",
		name, SOC_CONTROL_GET_ID_GET(idx),
		SOC_CONTROL_GET_ID_PUT(idx), SOC_CONTROL_GET_ID_INFO(idx));
}
	...
	struct snd_soc_dapm_widget *w, struct snd_soc_tplg_dapm_widget *tplg_w)
{
	...
	dev_dbg(tplg->dev, "ASoC: no handler specified for ext widget %s\n",
		w->name);
	return 0;
}

/* pass dynamic FEs configurations to component driver */
static int soc_tplg_fe_load(struct soc_tplg *tplg,
	struct snd_soc_tplg_pcm_fe *fe, int num_fe)
{
	if (tplg->comp && tplg->ops && tplg->ops->fe_load)
		return tplg->ops->fe_load(tplg->comp, fe, num_fe);

	dev_dbg(tplg->dev, "ASoC: no handler specified for FEs\n");
	return 0;
}

/* pass dynamic BEs configurations to component driver */
static int soc_tplg_be_load(struct soc_tplg *tplg,
	struct snd_soc_tplg_pcm_be *be, int num_be)
{
	if (tplg->comp && tplg->ops && tplg->ops->be_load)
		return tplg->ops->be_load(tplg->comp, be, num_be);

	dev_dbg(tplg->dev, "ASoC: no handler specified for BEs\n");
	return 0;
}

/* pass dynamic BEs configurations to component driver */
static int soc_tplg_cc_load(struct soc_tplg *tplg,
	struct snd_soc_tplg_pcm_cc *cc, int num_cc)
{
	if (tplg->comp && tplg->ops && tplg->ops->cc_load)
		return tplg->ops->cc_load(tplg->comp, cc, num_cc);

	dev_dbg(tplg->dev, "ASoC: no handler specified for Codec <-> Codec\n");
	return 0;
}
...
/* remove BE configurations */
static void remove_be(struct snd_soc_component *comp,
	struct snd_soc_dobj *dobj, int pass)
{
	if (pass != SOC_TPLG_PASS_BE)
		return;

	if (dobj->ops && dobj->ops->be_unload)
		dobj->ops->be_unload(comp, dobj);

	list_del(&dobj->list);
	kfree(dobj);
}

/* remove FE configurations */
static void remove_fe(struct snd_soc_component *comp,
	struct snd_soc_dobj *dobj, int pass)
{
	if (pass != SOC_TPLG_PASS_FE)
		return;

	if (dobj->ops && dobj->ops->fe_unload)
		dobj->ops->fe_unload(comp, dobj);

	list_del(&dobj->list);
	kfree(dobj);
}

/* remove Codec <-> Codec configurations */
static void remove_cc(struct snd_soc_component *comp,
	struct snd_soc_dobj *dobj, int pass)
{
	if (pass != SOC_TPLG_PASS_CC)
		return;

	if (dobj->ops && dobj->ops->cc_unload)
		dobj->ops->cc_unload(comp, dobj);
...
static int soc_tplg_kcontrol_bind_io(u32 io_type, struct snd_kcontrol_new *k,
...
static int soc_tplg_init_kcontrol(struct soc_tplg *tplg,
	struct snd_kcontrol_new *k, struct snd_soc_tplg_control_hdr *hdr)
{
	if (tplg->comp && tplg->ops && tplg->ops->control_load)
		return tplg->ops->control_load(tplg->comp, k, hdr);

	dev_dbg(tplg->dev, "ASoC: no handler specified for kcontrol %s\n",
		k->name);
	return 0;
}
	...
	struct snd_kcontrol_new *kc, u32 tlv_size)
{
	...
	dev_dbg(tplg->dev, " created TLV type %d size %d bytes\n",
		tplg_tlv->numid, tplg_tlv->length);
	tlv->numid = tplg_tlv->numid;
	tlv->length = tplg_tlv->length;
	memcpy(tlv->tlv, tplg_tlv + 1, tplg_tlv->length);
	kc->tlv.p = (void *)tlv;
	...
	size_t size)
{
	...
	if (soc_tplg_check_elem_count(tplg,
		sizeof(struct snd_soc_tplg_bytes_ext), count,
			size, "mixer bytes")) {
	...
	for (i = 0; i < count; i++) {
		be = (struct snd_soc_tplg_bytes_ext *)tplg->pos;
		...
		tplg->pos += (sizeof(struct snd_soc_tplg_bytes_ext) +
			be->priv.length);
		...
		ext = soc_tplg_kcontrol_bind_io(be->hdr.index, &kc, io_ops,
			ARRAY_SIZE(io_ops));

		if (ext) {
			/* none exist, so now try and map ext handlers */
			ext = soc_tplg_kcontrol_bind_io(be->hdr.index, &kc,
				tplg->io_ops, tplg->io_ops_count);
			if (ext) {
				soc_control_err(tplg, be->hdr.name,
					be->hdr.index);
				kfree(sbe);
				continue;
			}
		...
		err = soc_tplg_init_kcontrol(tplg, &kc,
			(struct snd_soc_tplg_control_hdr*)be);
		if (err < 0) {
	...
	size_t size)
{
	...
	for (i = 0; i < count; i++) {
		...
		/* map standard io handlers and check for external handlers */
		ext = soc_tplg_kcontrol_bind_io(mc->hdr.index, &kc, io_ops,
			ARRAY_SIZE(io_ops));
		if (ext) {
			/* none exist, so now try and map ext handlers */
			ext = soc_tplg_kcontrol_bind_io(mc->hdr.index, &kc,
				tplg->io_ops, tplg->io_ops_count);
			if (ext) {
				soc_control_err(tplg, mc->hdr.name,
					mc->hdr.index);
				kfree(sm);
				continue;
			}
			...
			/* pass control to driver for optional further init */
			err = soc_tplg_init_kcontrol(tplg, &kc,
				(struct snd_soc_tplg_control_hdr*) mc);
			if (err < 0) {
				dev_err(tplg->dev, "ASoC: failed to init %s\n",
					mc->hdr.name);
				kfree(sm);
	...
	size_t size)
{
	...
	for (i = 0; i < count; i++) {
		...
		/* map standard io handlers and check for external handlers */
		ext = soc_tplg_kcontrol_bind_io(ec->hdr.index, &kc, io_ops,
			ARRAY_SIZE(io_ops));
		if (ext) {
			/* none exist, so now try and map ext handlers */
			ext = soc_tplg_kcontrol_bind_io(ec->hdr.index, &kc,
				tplg->io_ops, tplg->io_ops_count);
			if (ext) {
				soc_control_err(tplg, ec->hdr.name,
					ec->hdr.index);
				kfree(se);
				continue;
			}
		...
		/* pass control to driver for optional further init */
		err = soc_tplg_init_kcontrol(tplg, &kc,
			(struct snd_soc_tplg_control_hdr*) ec);
		if (err < 0) {
			dev_err(tplg->dev, "ASoC: failed to init %s\n",
				ec->hdr.name);
			kfree(se);
...
static int soc_tplg_kcontrol_elems_load(struct soc_tplg *tplg,
	struct snd_soc_tplg_hdr *hdr)
{
	struct snd_soc_tplg_elems *elems =
		(struct snd_soc_tplg_elems *)tplg->pos;
	struct snd_soc_tplg_control_hdr *control_hdr;
	int i;

	if (tplg->pass != SOC_TPLG_PASS_MIXER) {
		tplg->pos += sizeof(struct snd_soc_tplg_elems) + hdr->size;
		return 0;
	}

	tplg->pos += sizeof(struct snd_soc_tplg_elems);

	dev_dbg(tplg->dev, "ASoC: adding %d kcontrols\n", elems->count);

	for (i = 0; i < elems->count; i++) {

		control_hdr = (struct snd_soc_tplg_control_hdr *)tplg->pos;
		...
		case SOC_DAPM_TYPE_PIN:
			soc_tplg_dmixer_create(tplg, 1, hdr->size);
			break;
		case SOC_CONTROL_TYPE_ENUM:
		case SOC_CONTROL_TYPE_ENUM_EXT:
		case SOC_CONTROL_TYPE_ENUM_VALUE:
		...
		case SOC_DAPM_TYPE_ENUM_VALUE:
		case SOC_DAPM_TYPE_ENUM_EXT:
			soc_tplg_denum_create(tplg, 1, hdr->size);
			break;
		case SOC_CONTROL_TYPE_BYTES_EXT:
			soc_tplg_dbytes_create(tplg, 1, hdr->size);
			break;
		default:
			soc_bind_err(tplg, control_hdr->index, elems->count);
		}
	}
	...
	struct soc_tplg *tplg, int num_kcontrols)
{
	...
	for (i = 0; i < num_kcontrols; i++) {
		...
		/* map standard io handlers and check for external handlers */
		ext = soc_tplg_kcontrol_bind_io(mc->hdr.index, &kc[i], io_ops,
			ARRAY_SIZE(io_ops));
		if (ext) {
			/* none exist, so now try and map ext handlers */
			ext = soc_tplg_kcontrol_bind_io(mc->hdr.index, &kc[i],
				tplg->io_ops, tplg->io_ops_count);
			if (ext) {
				soc_control_err(tplg, mc->hdr.name,
					mc->hdr.index);
	...
	struct soc_tplg *tplg, int count)
{
	...
	for (i = 0; i < count; i++) {
		...
		/* map standard io handlers and check for external handlers */
		ext = soc_tplg_kcontrol_bind_io(be->hdr.index, &kc[i], io_ops,
				ARRAY_SIZE(io_ops));
		if (ext) {
			/* none exist, so now try and map ext handlers */
			ext = soc_tplg_kcontrol_bind_io(be->hdr.index, &kc[i],
					tplg->io_ops, tplg->io_ops_count);
	...
	struct snd_soc_tplg_dapm_widget *w)
{
	...
	template.dobj.id = tplg->id;

	tplg->pos +=
		(sizeof(struct snd_soc_tplg_dapm_widget) + w->priv.length);
	...
	case SOC_DAPM_TYPE_VOLSW:
		...
		template.kcontrol_news = soc_tplg_dapm_widget_dmixer_create(tplg,
	...
	case SOC_DAPM_TYPE_ENUM_EXT:
		...
		template.kcontrol_news = soc_tplg_dapm_widget_denum_create(tplg);
		if (!template.kcontrol_news) {
			ret = -ENOMEM;
			goto hdr_err;
		}
	...
	struct snd_soc_tplg_hdr *hdr)
{
	...
	if (soc_tplg_check_elem_count(tplg,
		sizeof(struct snd_soc_tplg_dapm_graph_elem), count,
		hdr->size, "widget")) {

		dev_err(tplg->dev, "ASoC: invalid count %d for widgets\n",
			count);
		return -EINVAL;
	}
...
static int soc_tplg_coeff_load(struct soc_tplg *tplg,
	struct snd_soc_tplg_hdr *hdr)
{
	struct snd_soc_tplg_elems *elems =
		(struct snd_soc_tplg_elems *)tplg->pos;
	struct snd_soc_tplg_control_hdr *control_hdr;
	struct snd_soc_tplg_hdr *vhdr;
	int ret;

	if (tplg->pass != SOC_TPLG_PASS_COEFF)
		return 0;
	...
	dev_dbg(tplg->dev, "ASoC: got %d new coefficients\n", elems->count);
	...
	case SOC_CONTROL_TYPE_ENUM_EXT:
	case SOC_CONTROL_TYPE_ENUM_VALUE:
		ret = soc_tplg_denum_create(tplg, 1, hdr->size);
		if (ret < 0) {
			dev_err(tplg->dev, "ASoC: failed to create coeff enum %d\n",
				ret);
			return ret;
		}
		break;
	default:
		dev_err(tplg->dev, "ASoC: invalid coeff control type %d count %d\n",
			SOC_CONTROL_GET_ID_INFO(control_hdr->index),
			elems->count);
		return -EINVAL;
	}

	vhdr = (struct snd_soc_tplg_hdr *)tplg->pos;

	ret = soc_tplg_vendor_load_(tplg, vhdr);
	if (ret < 0) {
		dev_err(tplg->dev, "ASoC: unabled to load coeff data %d\n",
			ret);
		return ret;
	}
	tplg->pos += sizeof(*vhdr) + vhdr->size;
	vhdr = (struct snd_soc_tplg_hdr *)tplg->pos;

	return 0;
}

static int soc_tplg_be_elems_load(struct soc_tplg *tplg,
	struct snd_soc_tplg_hdr *hdr)
{
	struct  snd_soc_tplg_elems *elems =
		(struct snd_soc_tplg_elems *)tplg->pos;
	...
	if (tplg->pass != SOC_TPLG_PASS_BE)
		return 0;

	tplg->pos += sizeof(struct snd_soc_tplg_elems);
	be = (struct snd_soc_tplg_pcm_be *)tplg->pos;

	if (soc_tplg_check_elem_count(tplg,
		sizeof(struct snd_soc_tplg_pcm_be), count, hdr->size, "BE")) {
		dev_err(tplg->dev, "ASoC: invalid count %d for BE elems\n",
			count);
		return -EINVAL;
	}

	dev_dbg(tplg->dev, "ASoC: adding %d BEs\n", count);
	tplg->pos += sizeof(struct snd_soc_tplg_pcm_be) + count;
	...
	ret = soc_tplg_be_load(tplg, be, count);
	if (ret < 0) {
		dev_err(tplg->comp->dev, "ASoC: BE loading failed\n");
		goto err;
	}

	dobj->type = SND_SOC_DOBJ_BE;
	dobj->be.count = count;
	dobj->be.be = be;
	dobj->ops = tplg->ops;
	dobj->id = tplg->id;
...
static int soc_tplg_fe_elems_load(struct soc_tplg *tplg,
	struct snd_soc_tplg_hdr *hdr)
{
	struct  snd_soc_tplg_elems *elems =
		(struct snd_soc_tplg_elems *)tplg->pos;
	...
	if (tplg->pass != SOC_TPLG_PASS_FE)
		return 0;

	tplg->pos += sizeof(struct snd_soc_tplg_elems);

	if (soc_tplg_check_elem_count(tplg, sizeof(struct snd_soc_tplg_pcm_fe),
				count, hdr->size, "FE")) {
		dev_err(tplg->dev, "ASoC: invalid count %d for FE elems\n",
			count);
		return -EINVAL;
	}

	fe = (struct snd_soc_tplg_pcm_fe *)tplg->pos;

	dev_dbg(tplg->dev, "ASoC: adding %d FE\n", count);
	tplg->pos += sizeof(struct snd_soc_tplg_pcm_fe) * count;
	...
	/* Call the platform driver call back to register the dais */
	ret = soc_tplg_fe_load(tplg, fe, count);
	if (ret < 0) {
		dev_err(tplg->comp->dev, "ASoC: failed to add FE\n");
		goto err;
	}

	dobj->type = SND_SOC_DOBJ_FE;
	dobj->fe.count = count;
	dobj->fe.fe = fe;
	dobj->ops = tplg->ops;
	dobj->id = tplg->id;
...
static int soc_tplg_cc_elems_load(struct soc_tplg *tplg,
	struct snd_soc_tplg_hdr *hdr)
{
	struct  snd_soc_tplg_elems *elems =
		(struct snd_soc_tplg_elems *)tplg->pos;
	struct snd_soc_tplg_pcm_cc *cc;
	int count = elems->count;
	struct snd_soc_dobj *dobj;
	int ret;

	if (tplg->pass != SOC_TPLG_PASS_CC)
		return 0;

	tplg->pos += sizeof(struct snd_soc_tplg_elems);

	if (soc_tplg_check_elem_count(tplg, sizeof(struct snd_soc_tplg_pcm_cc),
				count, hdr->size, "Codec <-> Codec")) {
		dev_err(tplg->dev, "ASoC: invalid count %d for Codec <-> Codec elems\n",
			count);
		return -EINVAL;
	}

	cc = (struct snd_soc_tplg_pcm_cc *)tplg->pos;

	dev_dbg(tplg->dev, "ASoC: adding %d Codec <-> Codec links\n", count);
	tplg->pos += sizeof(struct snd_soc_tplg_pcm_cc) * count;

	dobj = kzalloc(sizeof(struct snd_soc_dobj), GFP_KERNEL);
	if (dobj == NULL)
		return -ENOMEM;

	/* let the driver know the codec <-> codec link capabilities */
	ret = soc_tplg_cc_load(tplg, cc, count);
	if (ret < 0) {
		dev_err(tplg->comp->dev, "ASoC: failed to add Codec <-> Codec\n");
		goto err;
	}

	dobj->type = SND_SOC_DOBJ_CC;
	dobj->cc.count = count;
	dobj->cc.cc = cc;
	dobj->ops = tplg->ops;
	dobj->id = tplg->id;
	...
	struct snd_soc_tplg_hdr *hdr)
{
	...
	if (hdr->magic == cpu_to_be32(SND_SOC_TPLG_MAGIC)) {
		dev_err(tplg->dev,
			"ASoC: %s at pass %d big endian not supported header got %x at offset 0x%lx size 0x%zx.\n",
			tplg->file, tplg->pass, hdr->magic,
	...
	if (hdr->magic != SND_SOC_TPLG_MAGIC) {
		dev_err(tplg->dev,
			"ASoC: %s at pass %d does not have a valid header got %x at offset 0x%lx size 0x%zx.\n",
			tplg->file, tplg->pass, hdr->magic,
			soc_tplg_get_hdr_offset(tplg), tplg->fw->size);
		return -EINVAL;
	}
	...
	if (hdr->abi != SND_SOC_TPLG_ABI_VERSION) {
		dev_err(tplg->dev,
			"ASoC: %s at pass %d invalid ABI version got 0x%x need 0x%x at offset 0x%lx size 0x%zx.\n",
			tplg->file, tplg->pass, hdr->abi,
	...
	if (hdr->size == 0) {
		dev_err(tplg->dev, "ASoC: %s header has 0 size at offset 0x%lx.\n",
			tplg->file, soc_tplg_get_hdr_offset(tplg));
		return -EINVAL;
	}
	...
	struct snd_soc_tplg_hdr *hdr)
{
	...
	if (hdr->id != tplg->req_id && hdr->id != SND_SOC_TPLG_ID_ALL)
		return 0;
	...
	case SND_SOC_TPLG_BE:
		return soc_tplg_be_elems_load(tplg, hdr);
	case SND_SOC_TPLG_COEFF:
		return soc_tplg_coeff_load(tplg, hdr);
	case SND_SOC_TPLG_FE:
		return soc_tplg_fe_elems_load(tplg, hdr);
	case SND_SOC_TPLG_MANIFEST:
		return soc_tplg_manifest_load(tplg, hdr);
	case SND_SOC_TPLG_CC:
		return soc_tplg_cc_elems_load(tplg, hdr);
	default:
	...
	struct snd_soc_tplg_ops *ops, const struct firmware *fw, u32 id)
{
	...
	tplg.dai_ops = ops->dai_ops;
	tplg.dai_ops_count = ops->dai_ops_count;
	...
	u32 id)
{
	...
	list_for_each_entry_safe(w, next_w, &dapm->card->widgets, list) {
		...
		/* match ID */
		if (w->dobj.id != id && w->dobj.id != SND_SOC_TPLG_ID_ALL)
			continue;
...
int snd_soc_tplg_component_remove(struct snd_soc_component *comp, u32 id)
{
		...
		list_for_each_entry_safe(dobj, next_dobj, &comp->dobj_list,
			...
			switch (dobj->type) {
				...
				case SND_SOC_DOBJ_WIDGET:
					...
					break;
				case SND_SOC_DOBJ_FE:
					remove_fe(comp, dobj, pass);
					break;
				case SND_SOC_DOBJ_BE:
					remove_be(comp, dobj, pass);
					break;
				case SND_SOC_DOBJ_CC:
					remove_cc(comp, dobj, pass);
					break;
				default:
